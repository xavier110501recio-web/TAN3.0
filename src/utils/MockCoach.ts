...
export function generateMissionDebrief(
  outcome: "completed" | "completed_with_issue" | "partial",
  note: string,
  ...
  return {
    coachMessage: `${coachMessage}${nextMission ? ` ${formatMission(nextMission)}` : " The first sprint is complete. Take stock and set the next target."}`,
    nextMission,
    xpEarned: outcome === "partial" ? 0 : mission.xp,
    skillIncrement: { skillName: mission.skill, amount: outcome === "partial" ? 0 : 8 },
  };
}
...
export function generateCoachResponse(
  ...
  userProfile: User & { current_mission_title?: string },
): CoachResponse {
  void SYSTEM_PROMPT;
  ...
  return {
    response: pick(
      response || [
        `Okay. What's the specific problem right now? Not the whole business. Name the one constraint, then take the next action in Mission ${mission.mission_number}.`,
        `Bring it back to the next real action. For ${userProfile?.niche || "your niche"}, that means one market-facing move today.`,
        `No spiral. Identify the constraint, solve that one thing, and move.`,
      ],
      message,
    ),
  };
...
export function generatePlanSummary(userProfile: User): PlanSummary {
...
export function generateAlternativeMission(currentMission: Mission, userProfile: User, note = ""): Mission {
...
function formatMission(m: Mission): string {
  return `${m.title}: ${m.task} Why this matters: ${m.why} Skill: ${m.skill.replace("_", " ")}. XP: +${m.xp}.`;
}

function pick<T>(items: T[], seed: string | number = ""): T {
  return items[Math.abs([...String(seed)].reduce((a, c) => a + c.charCodeAt(0), 0)) % items.length];
}
